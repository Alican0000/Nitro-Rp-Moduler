...
stock Pkr_Evaluate(const gameId)
{
    ...
    Pkr_SendFormattedGameMessage(gameId, COLOR_GREY, "'/pkr nexthand' yazarak yeni bir el baþlatabilirsin.");
    return;
}
...
static stock PkrSys_AssignBlinds(const gameId)
{
    ...
    Pkr_SendFormattedGameMessage(gameId, COLOR_RED, "%s is the big blind $%d.", ReturnName(Pkr_GetPlayerId(gameId, _bigBlindPosition)), Pkr_GetBigBlind(gameId));
...
PkrCMD_SetTimer(const playerid, const parameters[]) {
	...
	if(amount == 0)
		Pkr_SendFormattedGameMessage(gameId, COLOR_RED, "%s has disabled the timer.", ReturnName(playerid));
	else
		Pkr_SendFormattedGameMessage(gameId, COLOR_RED, "%s has set the timer to: %d.", ReturnName(playerid), amount);
	return;
}
